...
const { getLayers, getSortedListFromLayers, quickSort } = require('../quickSort');
...
describe('#quickSort', () => {
    describe('given a list of numbers', () => {
        it('returns the sorted list', () => {
            const sortedList1 = quickSort([4, 6, 1, 2, 3]);
            expect(sortedList1).toStrictEqual([1, 2, 3, 4, 6]);

            const sortedList2 = quickSort([11, 10, 13, 1, 12, 7, 19, 16, 18, 20]);
            expect(sortedList2).toStrictEqual([1, 7, 10, 11, 12, 13, 16, 18, 19, 20]);

            const sortedList3 = quickSort([14, 21, 6, 20, 24, 4, 17, 30, 25, 10, 11, 16, 1, 18, 9, 27, 22, 19, 26, 5]);
