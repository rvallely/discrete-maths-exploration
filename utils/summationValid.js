exports.checkSumArgsValid = (args) => {
    ...
    if (/^-{0,1} *[0-9]+$/.test(operation)) {
        return {status: true, n: n, i: i, operation: [operation]};
    }
    else {
        for(let i = 0; i < operation.length; i++) {
            if (/[0-9]/.test(operation[i]) === false && operation[i] !== 'i' && /[+*/^-]/.test(operation[i]) === false) {
                    return { status: false, msg: 'Operation argument is invalid.'};
            }            
            else if (/[0-9]/.test(operation[i]) === true) {
                    if (formattedOperation.length === 0) {
                        ...
                        if (/[0-9]/.test(formattedOperation[formattedOperation.length-1]) === true) {
                            formattedOperation[formattedOperation.length-1] += operation[i];
                        }
                    ...
                    else if (formattedOperation.length === 0 && operation[i] === '-') {
                        formattedOperation.push(operation[i])
                    }
                    else if (formattedOperation.length > 0) {
    ...
    return {status: true, n: n, i: i, operation: formattedOperation};
}