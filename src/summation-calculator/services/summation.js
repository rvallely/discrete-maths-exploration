exports.convertToNums = (operation) => operation.map((element) => {
...
exports.expDivMult = (convertedOperation, i) => {
    ...
    while (convertedOperation.includes('*')) {
        const result = convertedOperation[convertedOperation.indexOf('*') - 1] * convertedOperation[convertedOperation.indexOf('*') + 1];
        convertedOperation.splice(convertedOperation.indexOf('*') - 1, 3, result);
    }
    while (convertedOperation.includes('/')) {
        const result = convertedOperation[convertedOperation.indexOf('/') - 1] / convertedOperation[convertedOperation.indexOf('/') + 1];
