...
describe('#sortListUsingSelectionSort', () => {
    describe('given a list of numbers', () => {
        it('returns the list sorted asc -> desc', () => {
            expect(sortListUsingSelectionSort([5, 2, 3, 1, 4]).sortedList).toStrictEqual([1, 2, 3, 4, 5]);
            expect(sortListUsingSelectionSort([4, 2, 7, 1, 3]).sortedList).toStrictEqual([1, 2, 3, 4, 7]);
            expect(sortListUsingSelectionSort([1]).sortedList).toStrictEqual([1]);
            expect(sortListUsingSelectionSort(
                [4, 5, 6, 0, 7, 9, 10, 1, 8, 3],
            ).sortedList).toStrictEqual([0, 1, 3, 4, 5, 6, 7, 8, 9, 10]);
            expect(sortListUsingSelectionSort(
